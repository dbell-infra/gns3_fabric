...
ip addr add 10.255.255.3/32 dev loop1
...
ip addr add 172.16.255.1/29 dev INET_PEERING
...
ip addr add 172.16.255.9/29 dev VPN1_PEERING
...
ip addr add 172.16.255.17/29 dev VPN2_PEERING
ip link set VPN2_PEERING up
#
#
#CREATE VXLAN SEGMENTS
#
ip link add vxlan100 type vxlan id 100 dstport 4789 local 10.255.255.3 nolearning
brctl addbr br100
brctl addif br100  vxlan100
brctl stp br100  off
ip link set up dev br100
ip link set up dev vxlan100
ip link add vxlan101 type vxlan id 101 dstport 4789 local 10.255.255.3 nolearning
brctl addbr br101
brctl addif br101  vxlan101
brctl stp br101  off
ip link set up dev br101
ip link set up dev vxlan101
ip link add vxlan102 type vxlan id 102 dstport 4789 local 10.255.255.3 nolearning
brctl addbr br102
